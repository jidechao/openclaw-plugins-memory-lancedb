...
export interface RetrievalConfig {
  ...
  bm25Weight: number;
  minScore: number;
  rerank: "cross-encoder" | "siliconflow" | "lightweight" | "none";
  ...
  rerankApiKey?: string;
  /** SiliconFlow API key (enables siliconflow reranking) */
  rerankSFApiKey?: string;
  /** Jina/SiliconFlow Reranker model */
  rerankModel?: string;
  /** Custom rerank API base URL (for siliconflow or other compatible APIs) */
...
export class MemoryRetriever {
  ...
  /**
   * Rerank results using Jina cross-encoder API, SiliconFlow API, or cosine similarity fallback.
   */
  private async rerankResults(query: string, queryVector: number[], results: RetrievalResult[]): Promise<RetrievalResult[]> {
    ...
    // Try SiliconFlow reranker
    if (this.config.rerank === "siliconflow" && this.config.rerankSFApiKey) {
      try {
        const documents = results.map(r => r.entry.text);
        const model = this.config.rerankModel || "BAAI/bge-reranker-v2-m3";
        const baseURL = this.config.rerankBaseURL || "https://api.siliconflow.cn/v1";

        // Timeout: 10 seconds for SiliconFlow API
        ...
        const response = await fetch(`${baseURL}/rerank`, {
          ...
          headers: {
            ...
            "Authorization": `Bearer ${this.config.rerankSFApiKey}`,
          },
          body: JSON.stringify({
            model,
            query,
        ...
        if (response.ok) {
          ...
          if (!Array.isArray(data.results)) {
            console.warn("SiliconFlow rerank: invalid response shape, falling back to cosine");
          } else {
            ...
            const reranked = data.results
              ...
              .map(item => {
                ...
                // Blend: 60% rerank score + 40% original fused score
          ...
          console.warn(`SiliconFlow rerank API returned ${response.status}: ${errorText}, falling back to cosine`);
        }
      } catch (error) {
        if (error instanceof Error && error.name === "AbortError") {
          console.warn("SiliconFlow rerank timed out (10s), falling back to cosine");
        } else {
          console.warn("SiliconFlow rerank failed, falling back to cosine:", error);
        }
      }
    }
    // Try Jina cross-encoder reranker
    else if (this.config.rerank === "cross-encoder" && this.config.rerankApiKey) {
      try {
        const documents = results.map(r => r.entry.text);
